...
# Start Elasticsearch, FluentD and Kibana services
#
...
rm -f /var/run/elasticsearch/elasticsearch.pid /var/run/fluentd.pid \
  /var/run/kibana5.pid
...
if [ -z "$FLUENTD_START" ]; then
  FLUENTD_START=1
fi
if [ "$FLUENTD_START" -ne "1" ]; then
  echo "FLUENTD_START is set to something different from 1, not starting..."
else
...
if [ "$ELASTICSEARCH_START" -ne "1" ] && [ "$FLUENTD_START" -ne "1" ] \
